...
class FlightSimulator:
    ...
    def get_value(self, var_name: str):
        ...
        #if var_name == '/position/latitude-deg':
        #    self.variables[var_name] = var_value + 0.001 * math.cos(self.angle)
        #    self.angle += math.pi / 60
        #elif var_name == '/position/longitude-deg':
        #    self.variables[var_name] = var_value + 0.001 * math.sin(self.angle)
        #    self.angle += math.pi / 60
        #else:
        #    self.variables[var_name] = var_value + 0.001
        #####################
        iserr = random.randint(0, 2)
        if iserr == 1 and var_name != '/position/longitude-deg' and var_name != '/position/latitude-deg':
            self.variables[var_name] = "ERR"
        elif self.variables[var_name] == "ERR" and var_name != '/position/longitude-deg' and var_name != '/position/latitude-deg':
            min_range, max_range = self.variable_ranges[var_name]
            self.variables[var_name] = min_range + 0.001
        else:
            self.variables[var_name] = var_value + 0.001
        ######################

        #self.variables[var_name] = var_value+0.002 * random.random() + 0.001
        #iserr = random.randint(1,10)
        #return "ERR"
